...
public class ZIM {
  ...
  static edi_translate.ZIM_translate edi_decode = new edi_translate.ZIM_translate();
  ...
  public static string startUp(String operatorCode) {
    ...
    foreach(FileInfo fi in fpath.getedi_ZIM.GetFiles("*.*")) {
      fileName = fi.Name.ToString();
      try {

        EDIDATA = System.IO.File.ReadAllLines(fpath.getedi_ZIM.ToString() + fi.Name);

        if (EDIDATA.Length.ToString() == "1") {
          checkFile = File.ReadAllText(fpath.getedi_ZIM.ToString() + fi.Name.ToString());
          checkFile = checkFile.Replace("'", "'\r\n");
          File.WriteAllText(fpath.getedi_ZIM.ToString() + fi.Name.ToString(), checkFile);
        ...
        edi_version = edi_decode.checkEDIVersion(new string[] {
          EDIDATA[2]
        });
        ...
        if (edi_version == "COPARN:D:95B") 
        {
            ...
            // fi.MoveTo(Path.Combine(fpath.getedi_ZIM.ToString() + fi.Name.ToString(), fpath.backupedi_ZIM.ToString() + DateTime.Now.ToString("yyyyMMdd_HHmmss") + fi.Name.ToString()));
        ...
        else 
        {
            ...
            // File.Move(fpath.getedi_ZIM.ToString() + fi.Name.ToString(), fpath.moveedi_ZIM.ToString() + fi.Name.ToString());
            // fi.MoveTo(Path.Combine(fpath.getedi_ZIM.ToString() + fi.Name.ToString(), fpath.moveedi_ZIM.ToString() + DateTime.Now.ToString("yyyyMMdd_HHmmss") + fi.Name.ToString()));
            // fi.MoveTo(Path.Combine(fpath.moveedi_ZIM.ToString(), DateTime.Now.ToString("yyyyMMdd_HHmmss") + fi.Name.ToString()));
      ...
      if (dt.Rows.Count > 0) {
        ...
        if (dt.Rows[0]["Message Status"].ToString() == "Original") {
          for (int record = 0; record < dt.Rows.Count; record++) {
            ...
            if (columns.Contains("Temp")) {
              TMP = Int64.Parse(dt.Rows[record]["Temp"].ToString()).ToString();
            }
        ...
        if (dt.Rows[0]["Message Status"].ToString() == "Replace") {
          ...
          for (int record = 0; record < dt.Rows.Count; record++) {
            ...
            SqlDataAdapter sdaCheckExists = new SqlDataAdapter("SELECT  COUNT(*) FROM LCIT_EDI_COPARN.dbo.COPARN_EDIDATA_TEST WHERE Booking_No LIKE '" + dt.Rows[record]["Booking Reference"].ToString() + "' AND CNTR_NUM LIKE '" + dt.Rows[record]["Container No"].ToString() + "'", con);
            ...
            if (int.Parse(dtChekc.Rows[0][0].ToString()) > 0) {
              if (dt.Rows[record]["POR"].ToString() != "THBKK" || dt.Rows[record]["POR"].ToString() != "THLKR") 
              {
                if(DG_CLASS.ToString() == null || DG_CLASS.ToString() == "")
                {
                  ...
                  if(dt.Rows[record]["ISO"].ToString().Length == 4)
                  {
                      SqlDataAdapter SDAupdate = new SqlDataAdapter(" UPDATE LCIT_EDI_COPARN.dbo.COPARN_EDIDATA_TEST SET VESSEL_NAME = '" + dt.Rows[0]["Vessel Name"].ToString() + "', VOYAGE = '" + dt.Rows[0]["Voyage"].ToString() + "', OPERATOR_CODE = '" + dt.Rows[0]["Operator Code"].ToString() + "', POL = '" + dt.Rows[record]["POL"].ToString() + "', POD = '" + dt.Rows[record]["POD"].ToString() + "', DST = '" + dt.Rows[record]["DEL"].ToString() + "', CMDT = '" + CMDT + "', CNTR_NUM = '" + dt.Rows[record]["Container No"].ToString() + "', [SIZE] = '" + dt.Rows[record]["ISO"].ToString()+ "', [TYPE] = '', HEIGHT = '', SS = '" + dt.Rows[record]["Status"].ToString() + "', LADENT_SATAUS = '" + dt.Rows[record]["Laden"].ToString() + "', VGM = '" + VGM + "', VGM_UOM = '" + VGM_UOM + "', GROSSWEIGHT = '" + dt.Rows[record]["GROSSWEIGHT"].ToString() + "', GROSSWEIGHT_UOM = '" + dt.Rows[record]["GROSSWEIGHTUOM"].ToString() + "', SHIPPER_NAME = '" + SHIPPER_NAME + "', CONSIGNEE_NAME = '" + CONSIGNEE_NAME + "', SEAL = '" + SEAL + "', STOW_CODE = '" + STOW_CODE + "', BLOCK_STOWAGE = '" + BLOCK_STOWAGE + "', VENTILATION = '" + VENTILATION + "', TMP = '" + TMP + "', TMPUOM = '" + TMPUOM + "', DG_CLASS = '" + DG_CLASS + "', OOG_OH = '" + OOG_OH + "', OOG_OF = '" + OOG_OF + "', OOG_OA = '" + OOG_OA + "', OOG_OL = '" + OOG_OL + "', OOG_OR = '" + OOG_OR + "', GATE_ACTIVITY_CODE = 'PRE-ADVISE-" + refname.ToString() + "', PAYMENT = '" + PAYMENT + "', MODIFY_DATE = '" + DateTime.Now + "', RECORD_STATUS = '" + dt.Rows[0]["Message Status"].ToString() + "', ISASDRY ='" + ReeferType + "' WHERE Booking_No LIKE '" + dt.Rows[0]["Booking Reference"].ToString() + "'AND CNTR_NUM LIKE '" + dt.Rows[record]["Container No"].ToString() + "'", con);
